...
/**
 * Albums Component Model for a Album record.
 *
 * @package     Albums
 * @subpackage  com_albums
 * @since       3.0
 */
class AlbumsModelAlbum extends JModelItem
{
	/**
	 ...
	 * @var     string
	 */
	protected $_context = 'com_albums.album';
	...
	public function &getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($data))
				{
					throw new Exception(JText::_('COM_ALBUMS_ERROR_ALBUM_NOT_FOUND'), 404);
				}

				// Check for published state if filter set.
				if (((is_numeric($published)) || (is_numeric($archived))) && (($data->state != $published) && ($data->state != $archived)))
				{
					JError::raiseError(404, JText::_('COM_ALBUMS_ERROR_ALBUM_NOT_FOUND'));
				}
				...
				{
					$userId = $user->get('id');
					$asset  = 'com_albums.album.' . $data->id;
	...
	/**
	 * Method to increment the hit counter for the album.
	 *
	 * @param   int  $id  Optional ID of the album.
	 *
	...
	public function hit($id = null)
	{
		...
		$album = $this->getTable('Album', 'AlbumsTable');
