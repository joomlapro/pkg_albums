...
require_once JPATH_ADMINISTRATOR . '/components/com_albums/models/images.php';

/**
 * HTML Album View class for the Albums component.
 *
 * @package     Albums
 * @subpackage  com_albums
 * @since       3.1
...
class AlbumsViewAlbum extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		// Get an instance of the generic images model.
		$model = JModelLegacy::getInstance('Images', 'AlbumsModel', array('ignore_request' => true));
		$model->setState('list.select', 'a.*');
		$model->setState('list.ordering', 'a.ordering');
		$model->setState('list.direction', 'asc');
		$model->setState('filter.album_id', $this->item->id);

		$this->images = $model->getItems();
	...
	protected function _prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', JText::_('COM_ALBUMS_ALBUM_DEFAULT_PAGE_TITLE'));
		}
		...
		if ($menu && ($menu->query['option'] != 'com_albums' || $menu->query['view'] != 'album' || $id != $this->item->id))
		{
			...
			while ($category && ($menu->query['option'] != 'com_albums' || $menu->query['view'] == 'album' || $id != $category->id) && $category->id > 1)
			{
